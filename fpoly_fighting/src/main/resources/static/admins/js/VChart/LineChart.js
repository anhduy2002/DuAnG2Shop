class LineChart {
	...
	getSizeX = () => {
		...
		for( let i = 0 ; i < this.items.length ; i ++ ){
			const item = this.items[ i ]
			max = Math.max( max , item.value.toString().length , item.name.length )
		}
		return Math.max( _max , max * ( this.items.length - 1 ) * 0.55 * this.fontSize * 1.5 )
	}
	...
	show = () => {
		...
		this.itemSize = this.items.length == 1 ? 0 : this.size.x / ( this.items.length - 1 )
		...
		can.FLT( new Text( this.itemLabel , new P( this.p0.x + this.size.x * ( this.items.length + 1 ) / this.items.length , this.p0.y + this.fontSize * 1.5 ) , this.fontSize ) , "#333333" )
		...
		for( let i = 0 ; i < this.items.length ; i ++ ){
			...
			can.FLT( 
				new Text( 
					...
					new P( 
						this.p0.x + i * this.itemSize , 
						this.p0.y - item.value / this.maxValue * this.size.y - this.fontSize / 2 
					) 
				) , 
				"#333333" 
			)
		...
		can.MVT( 
			new P( 
				this.p0.x , 
				this.p0.y - this.items[ 0 ].value / this.maxValue * this.size.y 
			)
		)
		for( let i = 1 ; i < this.items.length ; i ++ ){
			const item = this.items[ i ]
